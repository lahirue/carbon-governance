...
import org.wso2.carbon.governance.taxonomy.stub.TaxonomyServicesRegistryExceptionException;
...
public class TaxonomyManagementClient {
    private static final Log log = LogFactory.getLog(TaxonomyManagementClient.class);
    private TaxonomyServicesStub stub;

    public TaxonomyManagementClient(String cookie, ServletConfig config, HttpSession session) throws RegistryException {
        String endpointURL;
    ...
    public boolean newTaxonomy(HttpServletRequest request) throws Exception {
        ...
        try {
            output = stub.addTaxonomy(payload);
     ...
     * @throws TaxonomyServicesRegistryExceptionException
     */
    public String getTaxonomy(HttpServletRequest request) throws RemoteException, TaxonomyServicesRegistryExceptionException {
        String taxonomyName = request.getParameter("taxonomyName");

        String output = null;
        try {
            output = stub.getTaxonomy(taxonomyName);
    ...
    public boolean updateTaxonomy (HttpServletRequest request)
            throws RemoteException, TaxonomyServicesRegistryExceptionException {
        String payload = request.getParameter("payload");
        String taxonomyName = request.getParameter("taxonomyName");
        boolean output = false;
        try {
            output = stub.updateTaxonomy(taxonomyName,payload);
    ...
    public boolean deleteTaxonomy (HttpServletRequest request) throws Exception {
        ...
        String taxonomyName = request.getParameter("taxonomyName");
        try {
            output = stub.deleteTaxonomy(taxonomyName);
        } catch (Exception e) {
            if (e instanceof RegistryException) {
                return false;
            } else {
